...
public class UltimateButton : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IDragHandler
{
	...
	public float customSpacing_X = -10.0f, customSpacing_Y = -10.0f;
	public RectTransform sizeFolder;
	public float fadeInDuration = -10.0f, fadeOutDuration = -10.0f;
	...
	void OnEnable ()
	{
		...
		// If this button is positioned relative to another transform, then start the coroutine to check for changes in the relative transform.
		if( positioning == Positioning.RelativeToTransform )
			StartCoroutine( RelativeTransformChecks() );
	...
	void OnDisable ()
	{
		...
		// If the positioning is relative to transform, then stop the coroutine.
		if( positioning == Positioning.RelativeToTransform )
			StopCoroutine( RelativeTransformChecks() );
	...
	public void OnPointerDown ( PointerEventData touchInfo )
	{
		...
		// If the user wants to exclusively use touch input, then return.
		if( useTouchInput )
			return;

		// Call the process down function with the current information.
	...
	public void OnPointerUp ( PointerEventData touchInfo )
	{
		...
		ProcessOnInputUp( touchInfo.pointerId );
	}
		...
		while( useTouchInput )
		{
			...
			if( Input.touchCount > 0 )
			{
				...
				for( int fingerId = 0; fingerId < Input.touchCount; fingerId++ )
				{
					...
					{
						// If the touch input position is within the bounds of the button rect, then process the down input on the button.
						if( buttonRect.Contains( Input.GetTouch( fingerId ).position ) )
					...
					// Else if the input has ended or if it was canceled, then process the input being released.
						...
						ProcessOnInputUp( fingerId );
				}
			}
			// Else there are no touches on the screen.
			else
			{
				// If the inputId is not reset then reset the button since there are no touches.
				if( _inputId > -10 )
					ResetButton();
			}
	...
	/// <summary>
	/// Processes the input when it has been initiated on the button.
	/// </summary>
	/// <param name="inputPosition">The position of the input on the screen.</param>
	/// <param name="inputId">The unique id of the input that has been initiated on the button.</param>
	void ProcessOnInputDown ( Vector2 inputPosition, int inputId )
	{
		...
		// Store the input id so that other functions can make sure the input is the same.
		_inputId = inputId;
	...
	void ProcessOnInputUp ( int inputId )
	{
		// If the pointer event that is calling this function is not the same as the one that initiated the button, then return.
		if( inputId != _inputId )
			return;
	...
	/// <summary>
	/// This coroutine checks for changes in the relative transform, and repositions accordingly.
	/// </summary>
	IEnumerator RelativeTransformChecks ()
	{
		// Store the relative transform information.
		Vector2 relativeTransformSize = relativeTransform.sizeDelta;
		Vector3 relativeTransformPos = relativeTransform.position;
